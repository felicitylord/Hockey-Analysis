...
shinyServer(function(input, output) {
  ...
  output$agsPlot <- renderPlot({
    ...
    ggplot(data = att_subset(), aes(x = factor(Category, levels = c("GAE_Dribble", "GAE_Pass", "E_Strong", "GAC_Dribble", "GAC_Pass", "C_Strong", "SP_High", "Direct", "Poss")), 
      ...
      geom_tile() +
      theme_bw() +
      scale_fill_gradient2(name = "Percentage\nof matches", low = "red", mid = "white", high = "lime green", midpoint = 50) +
  ...
  output$dgsPlot <- renderPlot({
    ...
    ggplot(def_subset(), aes(x = factor(Category, levels = c("GAE_Dribble", "GAE_Pass", "ED", "GAC_Dribble", "GAC_Pass", "CD", "SP_High", "Direct", "Poss")), 
      ...
      theme(plot.title = element_text(hjust = 0.5, size = 14), 
            ...
            legend.title = element_text(size = 14),
            legend.text = element_text(size = 12)) +
      scale_x_discrete(name = "", labels = c("GAE_Dribble" = "Opp Dribble", 
                                             "GAE_Pass" = "Opp Pass", 
                                             "ED" = "Strong", 
                                             "GAC_Dribble" = "Opp Dribble", 
                                             "GAC_Pass" = "Opp Pass", 
                                             "CD" = "Strong", 
                                             "SP_High" = "Opp High", 
                                             "Direct" = "Opp Direct", 
  ...
  output$goalsPlot <- renderPlot({
    ...
    ggplot() +
      ...
      theme_bw() +
      xlab("") +
      ggtitle("Goals For and Against") +
      theme(axis.text.x = element_text(angle = 90, size = 14),  
            legend.text = element_text(size = 14), 
            axis.title.y = element_text(size = 14)) +
      scale_fill_manual(name = "", labels = c(input$team4, "Opposition"), 
                        values = alpha(c("lime green", "red"), 0.75))
  ...
  output$ecPlot <- renderPlot({
    ...
    ggplot() +
      ...
      theme(axis.text.x = element_text(angle = 90, size = 14), 
            ...
            legend.text = element_text(size = 14), 
            legend.title = element_text(size = 14)) +
      scale_fill_manual(name = "Game Style Average", values = alpha(c("orange red", "dodger blue"), 0.75)) +
  ...
  output$tcPlot <- renderPlot({
    ...
    ggplot() +
      ...
      theme(axis.text.x = element_text(angle = 90, size = 14), 
            ...
            legend.text = element_text(size = 14), 
            legend.title = element_text(size = 14)) +
      scale_fill_manual(name = "Game Style Average", 
                        values = alpha(c("orange red", "dodger blue"), 0.75)) +
      scale_shape_manual(name = "Team Average", values = c(15, 16, 17)) +
      guides(shape = guide_legend(order = 1), fill = guide_legend(order = 2))
  ...
  output$bmPlot <- renderPlot({
    ...
    ggplot(bm_subset(), aes(x = factor(Variable, levels = c("Game_Poss", "Entropy", "Poss", "Back", "Stay", "Forward", "Goal")), 
      ...
      geom_tile() +
      theme_bw() +
      scale_fill_gradient2(name = "Comparison to\nleague average", low = "red", mid = "white", high = "lime green") +
  ...
  output$gsnPlot <- renderPlot({
    ...
    ggplot(gsn_subset(), aes(x = Loc.x, y = Loc.y)) +
      ...
      xlab("") +
      ylab("") +
      scale_fill_gradient2(name = "Comparison to\nleague average", low = "red", mid = "white", high = "lime green", midpoint = 0) +
  ...
  output$pcPlot <- renderPlot({
    ...
    ggplot(sp_subset(), aes(Pattern, Percent, group = factor(Outcome, levels = c("P","G", "Sa", "Sm", "T")))) + 
      ...
      scale_fill_manual(name = "Outcome", values = c("black", "lime green", "dodger blue", "navy blue", "red"),  
  ...
  output$gaPlot <- renderPlot({
    ...
    ggplot() +
      ...
      theme(axis.ticks.x = element_blank(), 
            ...
            legend.text = element_text(size = 14),
            strip.text = element_text(size = 14)) +
      scale_fill_manual(values = alpha(c("lime green", "dodger blue", "red"), 0.9)) +
  ...
  output$sPlot <- renderPlot({
    ...
    ggplot() +
      ...
      theme(axis.ticks.x = element_blank(), 
            ...
            legend.text = element_text(size = 14),
            strip.text = element_text(size = 14)) +
      scale_fill_manual(values = alpha(c("orange red", "navy blue", "dodger blue", "red", "lime green"), 0.9)) +
  ...
  output$tPlot <- renderPlot({
    ...
    ggplot() +
      ...
      theme_bw() +
      ylab("Turnover Type (%)") +
      scale_fill_manual(values = alpha(c("red", "blue",  "green2", "firebrick2", "dodger blue",  "green4", "orange red", "navy blue", "lime green"), 0.9)) +
  ...
  output$timePlot <- renderPlot({
    ...
    ggplot() +
      ...
      scale_fill_manual(name = "Match Status", values = alpha(c("dodger blue", "red", "lime green"), 0.85)) +
  ...
  output$endPlot <- renderPlot({
    ...
    ggplot() +
      ...
      ylab("Number of Possessions (%)") +
      theme_bw() +
      scale_fill_manual(name = "End Location", values = alpha(c("green2", "navy blue", "red", "lime green", "dodger blue", "orange red" ), 0.85)) +
      scale_shape_manual(name = "League Average", values = c(15, 16, 17), labels = c("Drawing", "Losing", "Winning")) +
      theme(axis.text.x = element_blank(), 
            axis.ticks.x = element_blank(),
            axis.title.y = element_text(vjust = 2.5, size = 14),
